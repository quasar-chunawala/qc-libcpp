...
template <typename T, typename... Ts>
requires type_lists::out_of_bounds_check<T, Ts...> constexpr size_t
    find_index_of_v = type_lists::find_index_of_impl<T, Ts...>();

template <typename T, typename... Ts> concept exists = requires {
...
requires(n < sizeof...(Ts)) using get_nth_type_t =
    typename decltype(type_lists::get_nth_type_impl<n, Ts...>())::type;
} // namespace tools
...
template <std::size_t i, typename Self>
    constexpr auto &&get(Self &&self) requires
    instance_of<std::remove_cvref_t<Self>, union_> &&
...
template <typename... Ts> class variant {
  ...
  public:
	 ...
	 */
	template <tools::exists<Ts...> T> constexpr explicit variant(T &&x) {
	 ...
	 * alternative as @p other, copy-constructed from the contained value.
	 */
	constexpr variant(const variant &other) {
		...
		[&]<auto... Indices>(std::index_sequence<Indices...>) {
			(((idx == Indices) ? (tools::construct_at<Indices>(
			                          m_data,
			                          tools::get<Indices>(other.m_data)),
			                      0)
			                   : 0),
			 ...);
		}(idx_seq);
	...
	constexpr ~variant() {
		constexpr auto idx_seq = std::make_index_sequence<sizeof...(Ts)>();
		[&]<auto... Indices>(std::index_sequence<Indices...>) {
			(((m_idx == Indices) ? (tools::destroy_at<Indices>(m_data), 0)
			                     : 0),
			 ...);
		}(idx_seq);
	}
