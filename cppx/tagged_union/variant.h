...
// out of bounds
template<typename T, typename... Ts>
concept out_of_bounds_check = (find_index_of_impl<T, Ts...>() < sizeof...(Ts));
} // namespace type_lists
...
static_assert(
  std::is_same_v<uint32_t,
                 dev::tools::uint_atleast_t<std::numeric_limits<uint32_t>::max()>>);
...
// -------------- tests ------------
static_assert(
  std::is_same_v<dev::tools::get_nth_type_t<0, char, short, int, long, float, double>,
                 char>);

static_assert(
  std::is_same_v<dev::tools::get_nth_type_t<5, char, short, int, long, float, double>,
                 double>);
...
struct union_size<union_<Ts...>> : std::integral_constant<size_t, sizeof...(Ts)>
{};
...
constexpr void
construct_at(union_<Ts...>& self, auto&&... args)
{
...
class variant
{
  ...
  public:
    ...
    constexpr variant(variant&& other) noexcept
    {
        ...
        [&]<auto... Indices>(std::index_sequence<Indices...>) {
            (((idx == Indices) ? (tools::construct_at<Indices>(
                                    m_data, tools::get<Indices>(std::move(other.m_data))),
                                  0)
                               : 0),
             ...);
        }(idx_seq);
